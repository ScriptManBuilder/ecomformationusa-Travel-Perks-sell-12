...
const Footer: React.FC = () => {
  const brandName = process.env.REACT_APP_BRAND_NAME || 'Trevlix';
  const companyName = process.env.REACT_APP_COMPANY_NAME || 'Ecomformationusa Inc';
  const companyAddress = process.env.REACT_APP_COMPANY_ADDRESS || '126 Lake Mariam Way, Winter Haven, FL 33885';
  const companyPhone = process.env.REACT_APP_PHONE_DISPLAY || '+17247506209';
  const supportEmail = process.env.REACT_APP_EMAIL_SUPPORT || 'support@ecomformationtravelsavings.com';
        ...
        <FooterGrid>
          <FooterSection>
            ...
            <CompanyInfo>
              <p>{companyName}</p>
              <p>{companyAddress}</p>
              <p>Established {process.env.REACT_APP_COMPANY_BIRTH || 'October 2025'}</p>
            </CompanyInfo>
